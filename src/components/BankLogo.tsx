...
function ChaseMark({ size }: { size: number }) {
  return (
    <div
      className="flex items-center justify-center shrink-0 rounded-lg overflow-hidden"
      style={{ width: size, height: size, backgroundColor: '#117ACA' }}
    >
      <svg
        viewBox="0 0 24 24"
        width={size * 0.58}
...
function CapitalOneMark({ size }: { size: number }) {
  return (
    <div
      className="flex items-center justify-center shrink-0 rounded-lg overflow-hidden border border-border"
      style={{ width: size, height: size, backgroundColor: '#ffffff' }}
    >
      <svg
        viewBox="0 0 418 150"
        width={size * 0.82}
        height={size * 0.82 * (150 / 418)}
        aria-label="Capital One"
        role="img"
      >
        <path
          d="M283.7,108.5h-.1c-17.8,12.6-38.8,25.8-61.5,39.7l-.9.5a.7.7,0,1,0,.8,1.1l.7-.4c19.3-10,41.4-21.9,63.6-34.2l.4-.2A24.7,24.7,0,0,1,283.7,108.5ZM400.5,13.9c-28-30.9-208.6-3.1-285.3,13.7l-1.8.4a.8.8,0,0,0-.6.8.6.6,0,0,0,.8.5l1.8-.3C179,17.7,311.3,2,339.7,30.2c8.7,8.6,6.7,19.7-3.4,32.8a25.7,25.7,0,0,1,11,15.8C386.6,52.7,413,27.7,400.5,13.9Z"
          fill="#cc2427"
        />
        <g fill="#013d5b">
...
function FallbackMark({ bankType, size }: { bankType: string; size: number }) {
  return (
    <div
      className="flex items-center justify-center shrink-0 rounded-lg bg-primary text-primary-foreground font-semibold"
      style={{ width: size, height: size, fontSize: size * 0.4 }}
    >
      {bankType.charAt(0).toUpperCase()}
    </div>
  );
}

export function BankLogo({ bankType, size = 40, className }: BankLogoProps) {
  const key = normalize(bankType);

  const content =
    key === 'chase' ? (
      <ChaseMark size={size} />
    ) : key === 'capitalone' ? (
      <CapitalOneMark size={size} />
    ) : (
      <FallbackMark bankType={bankType} size={size} />
    );
